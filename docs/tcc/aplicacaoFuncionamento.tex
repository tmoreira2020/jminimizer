...
Finalizando o processo temos a persistência da classe compactada e de todos os arquivos que estão disponíveis no classpath da dependência do tipo \textit{program} e que não são arquivos do tipo \textit{bytecode}, entre eles estão arquivos XML, figuras, etc. O programa final pode ser persistido num diretório ou em um arquivo do tipo jar\cite{jar}, essa configuração é feita no arquivo de configuração do projeto.
