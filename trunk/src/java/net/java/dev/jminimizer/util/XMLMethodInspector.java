...
public class XMLMethodInspector implements MethodInspector {
    ...
    private String buildPattern(Element element) throws ClassNotFoundException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            net.java.dev.jminimizer.beans.Class clazz = this
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (temp.startsWith("*") && temp.endsWith("*")) {
                ...
                buffer.append(ClassUtils.normalize(temp + "\\b"));
            } else if (temp.endsWith("*")) {
                buffer.append(ClassUtils.normalize("\\b" + temp));
            }
            if (i != (n - 1)) {
                buffer.append('|');
            }
    ...
    private Method buidMethod(String className, Element methodElement)
        ...
        Method m = new Method(className, methodElement.getAttribute("name"),
                args, ret, Boolean.valueOf(
                        methodElement.getAttribute("inSubClasses"))
                        .booleanValue());
        if (m.isInSubClasses()) {
    ...
    public boolean remove(MethodGen method) throws ClassNotFoundException {
        ...
        if (methodsNoRemove.contains(m.getNameAndSignature())) {
            if (ClassUtils.isMethodOverridFromSuperClass(method)) {
                log.debug("Method overrided from super class: "
    ...
    private void buildMethodsToInspect(Element root)
        ...
        if (inspect != null) {
            ...
            while (i.hasNext()) {
                net.java.dev.jminimizer.beans.Class clazz = (net.java.dev.jminimizer.beans.Class) i
                        .next();
